...
public class LoggingAspect {
    ...
    public Object afterReturningAdvice(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        logger.info("Logging time taken for method {}", proceedingJoinPoint.getSignature().getName());
    ...
    private void logTimeTaken(ProceedingJoinPoint proceedingJoinPoint, LocalDateTime before) {
        ...
        if (timeExceeded && operationsB)
            logger.warn(s.toString(), methodName, timeTaken, maxTime, (timeTaken - maxTime), operations);
        else if (maxTimeB && operationsB)
            logger.info(s.toString(), methodName, timeTaken, maxTime, operations);
        else if (maxTimeB)
            logger.info(s.toString(), methodName, timeTaken, maxTime);
        else if (operationsB)
            logger.info(s.toString(), methodName, timeTaken, operations);
        else
            logger.info(s.toString(), methodName, timeTaken);
    }
}